...
class MedicalGraph:
    def __init__(self):
        ...
        alphabets = ['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y', 'z']#用来避免像 a、b、c 这种单字符被当成有效词
        nums = ['1','2','3','4','5','6','7','8','9','0']#用来避免 1、2、3 这种内容进入实体结果
        ...
        for item in self.col.find():
            ...
            for attr, value in data.items():
                ...
                if attr_en in ['yibao_status', 'get_prob', 'easy_get', 'get_way', "cure_lasttime", "cured_prob"]:
                    ...
                    # No dictionary-based segmentation here; use a simple split fallback.
...
1. 先运行 data_spider.py，把 html/url 存进 MongoDB
2. 运行 build_data.py 里的 modify_jc()，解析检查项页面
3. 再运行 build_data.py 里的 collect_medical()，整理疾病数据
4. 最终得到结构化结果
