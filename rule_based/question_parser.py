...
class QuestionPaser:
    ...
    def build_entitydict(self, args):
        ...
        self.debug_print('entity_dict', entity_dict)
        return entity_dict

    '''解析主函数'''
    def parser_main(self, res_classify):
        self.debug_print('classify_input', res_classify)
        ...
        for question_type in question_types:
            ...
            if sql:
                sql_['sql'] = sql
                self.debug_print('sql_for_{0}'.format(question_type), sql)

                sqls.append(sql_)

        self.debug_print('parser_output', sqls)
        return sqls

    '''针对不同的问题，分开进行处理'''
    def sql_transfer(self, question_type, entities):
        self.debug_print('sql_transfer_input', {
            'question_type': question_type,
            'entities': entities,
        })
        if not entities:
            self.debug_print('sql_transfer_skip', 'No entities for {0}'.format(question_type))
            return []
