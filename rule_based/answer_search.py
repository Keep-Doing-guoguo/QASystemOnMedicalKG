...
class AnswerSearcher:
    ...
    def search_main(self, sqls):
        self.debug_print('search_input', sqls)
        final_answers = []
        for sql_ in sqls:
            question_type = sql_['question_type']
            queries = sql_['sql']
            self.debug_print('question_type', question_type)
            answers = []
            for query in queries:
                self.debug_print('cypher', query)
                ress = self.g.run(query).data()
                self.debug_print('raw_result', ress)
                answers += ress
            self.debug_print('merged_result', answers)
            final_answer = self.answer_prettify(question_type, answers)
            self.debug_print('pretty_answer', final_answer)
            if final_answer:
                final_answers.append(final_answer)
        self.debug_print('search_output', final_answers)
        return final_answers

    '''根据对应的qustion_type，调用相应的回复模板'''
    def answer_prettify(self, question_type, answers):
        self.debug_print('answer_prettify_input', {
