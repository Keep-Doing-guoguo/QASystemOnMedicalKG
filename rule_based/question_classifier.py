...
class QuestionClassifier:
    def __init__(self):
        ...
        # 加载特征词
        # Load all dictionaries as word lists (strip blanks).
        ...
        # Merge all entity words for global matching.
        ...
        # Negative words used to distinguish "not eat" from "do eat".
        self.deny_words = [i.strip() for i in open(self.deny_path, encoding='utf-8') if i.strip()]
        # 构造领域actree
        # Build Aho-Corasick automaton for fast multi-word matching.
        self.region_tree = self.build_actree(list(self.region_words))
        # 构建词典
        # Build word -> entity type(s) mapping.
    ...
    def classify(self, question):
        ...
        #收集问句当中所涉及到的实体类型
        # 2) Collect entity types appearing in the question.
        types = []
        for type_ in medical_dict.values():
            types += type_
        self.debug_print('matched_entities', medical_dict)
        self.debug_print('entity_types', types)
        question_type = 'others'

        question_types = []

        # 症状
        # 3) Determine intent(s) based on trigger words + entity types.
        ...
        # 推荐食品
        # Food recommendations (do_eat vs no_eat).
        ...
        # 疾病易感染人群
        # Easy-to-get population questions.
        if self.check_words(self.easyget_qwds, question) and 'disease' in types :
            question_type = 'disease_easyget'
            question_types.append(question_type)

        # 若没有查到相关的外部查询信息，那么则将该疾病的描述信息返回
        # Fallback: return description if only disease entity is found.
        if question_types == [] and 'disease' in types:
            question_types = ['disease_desc']

        # 若没有查到相关的外部查询信息，那么则将该疾病的描述信息返回
        # Fallback: if only symptom entity is found, ask which disease.
        ...
        self.debug_print('question_types', question_types)

        return data

    '''构造词对应的类型'''
    def build_wdtype_dict(self):
        # Map each word to one or more entity types.
        wd_dict = dict()
    ...
    def check_medical(self, question):
        # Find all matched words in the question.
        region_wds = []
        for i in self.region_tree.iter(question):
            wd = i[1][1]
            region_wds.append(wd)
        # Remove short words that are contained in longer matches.
        stop_wds = []#删掉不够具体的短词，保留更完整的实体词。
        for wd1 in region_wds:
            for wd2 in region_wds:
                if wd1 in wd2 and wd1 != wd2:
                    stop_wds.append(wd1)
        final_wds = [i for i in region_wds if i not in stop_wds]
        # Return {word: [type1, type2, ...]}.
    ...
    def check_words(self, wds, sent):
        # Simple keyword hit check for intents.
        for wd in wds:
            if wd in sent:
                return True
        return False
